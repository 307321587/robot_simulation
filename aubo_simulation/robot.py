...
class RobotBase(object):
    ...
    def get_ee_pose(self):
        ...
        end_pos= end_state[0]
        end_euler = R.from_quat(end_state[1]).as_euler('xyz')

        return end_pos, end_euler
    ...
    def move_ee(self, action, control_method):
        ...
        for i, joint_id in enumerate(self.arm_controllable_joints):
            p.setJointMotorControl2(self.id, joint_id, p.POSITION_CONTROL, joint_poses[i], maxVelocity=self.joints[joint_id].maxVelocity)
...
class Auboi16(RobotBase):
    def __init_robot__(self):
        # define the robot
        # see https://github.com/bulletphysics/bullet3/blob/master/examples/pybullet/gym/pybullet_robots/panda/panda_sim_grasp.py
        self.eef_id = 6
        self.arm_num_dofs = 6
        self.arm_rest_poses = [np.pi/2,0,-np.pi/2,0,-np.pi/2,0]
        self.id = p.loadURDF('./urdf/aubo_i16.urdf', self.base_pos, self.base_ori,
    ...
    def move_gripper(self, open_length):
        return
        # for i in [7, 8]:
        #     p.setJointMotorControl2(self.id, i, p.POSITION_CONTROL, open_length-0.04, force=20)
